...
import org.firewall.protectify.provider.Provider;
import org.firewall.protectify.provider.ProviderPicker;
import org.firewall.protectify.receiver.StatusBarBroadcastReceiver;
import org.firewall.protectify.server.AbstractDnsServer;
import org.firewall.protectify.server.DnsServer;
import org.firewall.protectify.server.DnsServerHelper;
import org.firewall.protectify.util.DnsServersDetector;
...
public class DaedalusVpnService extends VpnService implements Runnable {
    public static final String ACTION_ACTIVATE = "org.firewall.protectify.service.DaedalusVpnService.ACTION_ACTIVATE";
    public static final String ACTION_DEACTIVATE = "org.firewall.protectify.service.DaedalusVpnService.ACTION_DEACTIVATE";
    ...
    private static void updateUpstreamServers(Context context) {
        String[] servers = DnsServersDetector.getDnsServers(context);
        if (servers != null) {
    ...
    public void run() {
        try {
            ...
            Logger.info("Daedalus VPN service is listening on " + primaryServer.getAddress() + " as " +
                    (aliasPrimary != null ? aliasPrimary.getHostAddress() : "null"));
            Logger.info("Daedalus VPN service is listening on " + secondaryServer.getAddress() + " as " +
                    (aliasSecondary != null ? aliasSecondary.getHostAddress() : "null"));

            if (aliasPrimary != null) builder.addDnsServer(aliasPrimary);
            if (aliasSecondary != null) builder.addDnsServer(aliasSecondary);
